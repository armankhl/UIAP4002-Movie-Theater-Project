...
void EditWindow::setData()
{
	int Mnum = CountUsers4();
	QString Movie[Mnum];
	int i=0;
	QFile file("Movies.txt");
	file.open(QIODevice::ReadOnly);
...
void EditWindow::on_pushButton_1_clicked()
{
	QFile bfile("EditMovie.txt");
	bfile.open(QIODevice::WriteOnly);
	QTextStream bin(&bfile);
	bin << ui->label_1->text() << endl;
	bfile.close();

	EditSearchWindow *NbM = new EditSearchWindow();
	NbM->show();
	this->close();
}
